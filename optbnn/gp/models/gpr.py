...
class GPR(GPModel):
    ...
    def predict_f(self, Xnew, full_cov=False):
        ...
        A, _ = torch.linalg.solve(L, Kx)  # could use triangular solve, note gesv has B first, then A in AX=B
        V, _ = torch.linalg.solve(L, self.Y - self.mean_function(self.X)) # could use triangular solve
